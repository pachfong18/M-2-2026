...
const CASES_DB = [
    { 
        ...
        clues: ["1. 'เด็กชาย C' หิวมาก จึงเดินไปที่ 'โรงอาหาร'", "2. มีคนลืม 'แท็บเล็ต' ทิ้งไว้ที่ 'โรงอาหาร'", "3. 'นาง D' นั่งตรวจงานอยู่ที่ 'ห้องพักครู' ตลอดเวลา", "4. คนที่อยู่ 'ห้องพักครู' ใช้ 'แล็ปท็อป' ทำงาน", "5. ผู้ที่ขโมยข้อมูลแอบเข้าไปใน 'ห้องเซิร์ฟเวอร์'", "6. ผู้ที่ขโมยข้อมูลใช้ 'แฟลชไดรฟ์' เซฟไฟล์หนีไป", "7. 'นางสาว B' ไม่มี 'มือถือ' และไม่ได้ใช้ 'แท็บเล็ต'", "8. 'นาย A' ไม่เคยเดินไปที่ 'ห้องพักครู' หรือ 'โรงอาหาร'", "9. คนที่อยู่ 'สวน' ไม่ได้ใช้ 'แล็ปท็อป' และ 'แท็บเล็ต'", "10. รปภ. ยืนยันว่า 'นางสาว B' แอบเข้าไปใน 'ห้องเซิร์ฟเวอร์'"], 
    ...
    { 
        ...
        clues: ["1. 'ครูฝึกสอน' ไปคุมเด็กเล่นกีฬาที่ 'สนามบาส'", "2. มีคนลืม 'แว่นตาอัจฉริยะ' ไว้ที่ 'ดาดฟ้า'", "3. 'ภารโรง' กำลังทำความสะอาดอยู่ที่ 'ดาดฟ้า'", "4. คนที่นั่งใน 'ห้องสมุด' ใช้ 'โน้ตบุ๊ก' พิมพ์รายงาน", "5. 'ประธานสภา' นั่งเงียบๆ อยู่ใน 'ห้องสมุด'", "6. แฮกเกอร์ใช้ 'มินิพีซี' ในการแฮกระบบ", "7. แฮกเกอร์แอบต่อเน็ตจาก 'ห้องคอมฯ'", "8. คนที่อยู่ 'สนามบาส' ใช้ 'สมาร์ทวอทช์' จับเวลา", "9. 'หัวหน้าห้อง' ไม่ได้ใช้สมาร์ทวอทช์และโน้ตบุ๊ก", "10. 'หัวหน้าห้อง' แอบอยู่ในห้องคอมฯคนเดียว"], 
    ...
    { 
        ...
        clues: ["1. 'สมหญิง' นอนพักไข้ใน 'ห้องพยาบาล'", "2. คนที่อยู่ 'ห้องพยาบาล' ใช้ 'อีเมลสแปม' ส่งจดหมาย", "3. 'สมชาย' ซ้อมเป่าขลุ่ยอยู่ที่ 'ห้องดนตรี'", "4. คนที่อยู่ 'ห้องดนตรี' กำลังต่อ 'บลูทูธ' ลำโพง", "5. คนที่แอบอยู่ 'ใต้บันได' บังคับ 'โดรน' บินเล่น", "6. 'สมศักดิ์' แอบเล่นอยู่ 'ใต้บันได'", "7. ผู้กระทำผิด นั่งเสียบอุปกรณ์อยู่ที่ 'โต๊ะหินอ่อน'", "8. ไวรัสตัวนี้ถูกปล่อยผ่าน 'ทรัมบ์ไดรฟ์'", "9. 'สมปอง' ไม่ได้ไปห้องพยาบาล ดนตรี หรือใต้บันได", "10. 'สมปอง' เป็นคนเอาทรัมบ์ไดรฟ์มาโรงเรียน"], 
...
function viewTeacher(v) {
    ...
    if (v === 'students') {
        db.collection("students").where("studentId", "!=", TEACHER_ID).get().then(snap => {
            ...
            fList.forEach(s => {
                rows += `<tr>
                    <td><select class="edit-input" style="width:70px;" id="r_${s.studentId}"><option value="London" ${s.room==='London'?'selected':''}>London</option><option value="Newyork" ${s.room==='Newyork'?'selected':''}>Newyork</option><option value="Tokyo" ${s.room==='Tokyo'?'selected':''}>Tokyo</option><option value="Paris" ${s.room==='Paris'?'selected':''}>Paris</option><option value="Seoul" ${s.room==='Seoul'?'selected':''}>Seoul</option><option value="${s.room}" ${!ROOMS_LIST.includes(s.room)?'selected':''} style="display:${!ROOMS_LIST.includes(s.room)?'block':'none'}">${s.room}</option></select></td>
                    ...
                    <td><button class="btn-p" style="padding:8px; font-size:10px;" onclick="saveStudentProfile('${s.studentId}')">Save</button> <button class="btn-p btn-danger" style="padding:8px; font-size:10px;" onclick="deleteStudent('${s.studentId}','${s.name}')"><i class="fa-solid fa-trash"></i></button></td>
                </tr>`;
            });
            box.innerHTML = addForm + generateRoomTabs('students') + `<div style="overflow-x:auto;"><table class="admin-table">${header}${rows}</table></div>`;
        });
    }
    else if (v === 'grading') {
        db.collection("students").where("studentId", "!=", TEACHER_ID).get().then(snap => {
        ...
        let annHTML = announcements.map((a, i) => `<div style="background:rgba(255,255,255,0.05); border:1px solid #444; padding:15px; margin-bottom:10px; display:flex; gap:10px;"><input type="text" id="ann_edit_${i}" value="${a}" style="flex:1; margin:0;"><button class="btn-p" style="padding:10px; font-size:10px;" onclick="editAnnounce(${i})"><i class="fa-solid fa-save"></i></button><button class="btn-p btn-danger" style="padding:10px;" onclick="delAnnounce(${i})"><i class="fa-solid fa-trash"></i></button></div>`).join('');
        box.innerHTML = `<div style="background:rgba(255,153,0,0.1); border:1px solid #ff9900; padding:20px; border-radius:10px;"><div style="display:flex; gap:10px; margin-bottom:20px;"><input type="text" id="new-announce" placeholder="พิมพ์ประกาศใหม่ที่นี่..." onkeypress="if(event.key==='Enter') addAnnounce();"><button class="btn-p pixel-font" style="background:#ff9900; color:#000; font-size:10px;" onclick="addAnnounce()">เพิ่มประกาศ</button></div>${annHTML}</div>`;
    }
    else if(v === 'lessons') {
        ...
        box.innerHTML = `<div style="background:rgba(179,102,255,0.1); border:1px solid var(--detective-purple); padding:20px; border-radius:10px;"><h3 class="pixel-font" style="font-size:12px; color:var(--detective-purple);">ควบคุมแฟ้มคดีสืบสวน</h3><label style="font-size:12px;">เลือกคดีให้เด็กเล่น (มี 3 คดี)</label><select onchange="changeActiveCase(this.value)" style="background:#000; margin-bottom:20px;">${caseOptions}</select><p style="font-size:12px;">โชว์/ซ่อนคำใบ้ให้นักเรียนเห็น</p><div style="display:flex; flex-wrap:wrap; gap:10px; margin-bottom:20px;">${toggleBtns}</div><button class="btn-p" style="width:100%; background:${caseStatus.isRevealed?'var(--alert-red)':'var(--detective-purple)'};" onclick="toggleSetting('monthly_case', 'isRevealed')">${caseStatus.isRevealed ? 'ปิดการเฉลย' : '📢 กดปุ่มเฉลยให้เด็กเห็น'}</button></div>`;
    }
    else if(v === 'quests') {
...
function teacherCreateStudent() {
    const n = document.getElementById('tAddName').value.trim(); const id = document.getElementById('tAddId').value.trim();
...
// --- Helpers ---
function getCell(r, c) { let val = window.murdleState[`r${r}c${c}`] || ""; return `<td class="clickable ${val==='O'?'yes':(val==='X'?'no':'')}" style="border:1px solid #444;" onclick="clickGrid(${r},${c})">${val}</td>`; }
function clickGrid(r, c) { let k = `r${r}c${c}`; let curr = window.murdleState[k] || ""; let nxt = curr === "" ? "X" : (curr === "X" ? "O" : ""); window.murdleState[k] = nxt; let cell = document.getElementById(`cell_${r}_${c}`); cell.innerText = nxt; cell.className = "clickable " + (nxt==='O'?'yes':(nxt==='X'?'no':'')); }
...
function uploadDrive(inputId, missionId) {
    ...
    reader.onload = function() {
        ...
        fetch(GAS_URL, { method: "POST", body: JSON.stringify(payload) }).then(res => res.text()).then(resp => {
            if(resp === "Success") { let sm = userData.submittedMissions || []; sm.push(missionId); db.collection("students").doc(userData.studentId).update({ submittedMissions: sm }).then(() => openQuestDetail(missionId.split('_')[0])); } else { alert("Error: "+resp); btn.innerText = "UPLOAD"; btn.disabled = false; }
        });
    };
}
